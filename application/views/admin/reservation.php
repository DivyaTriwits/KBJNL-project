...
<script>
   ...
   function populateDropdownClass(){
     ...
     $class = $this->db->distinct('sapling_id')->select('sapling_id')->from('sapling_stock')->get()->result();
     ?>
     <?php foreach($class as $className){
        $saplings = $this->db->where('saplingid',$className->sapling_id)->get('saplings')->row();?>
       if (val !='') {
...
<script>
   ...
   function populateBagDropdownClass(){
     ...
     /*$citiename = $data->city;*/
    // $nurseryid= $("#name").val();
     $class = $this->db->select('bag_size.*,sapling_destils.*')->from('bag_size')->join('sapling_destils','sapling_destils.bag_size_id=bag_size.bag_id')->get()->result();
     ?>
     <?php foreach($class as $className){
        $bags = $this->db->distinct('bagsize')->where('bag_id',$className->bag_id)->get('bag_size')->row();?>
       if (val == '<?php echo $className->sapling_id?>') {
         $("#bag").append("<option value='<?php echo $bags->bag_id?>'><?php echo $className->bagsize?></option>");
      }
   <?php }?>
}
</script>