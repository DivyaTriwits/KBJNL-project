 <div class="row">
                    <div class="col-12  align-self-center">
                        <div class="sub-header mt-3 py-3 align-self-center d-sm-flex w-100 rounded">
                            <div class="w-sm-100 mr-auto"><h4 class="mb-0">Stocks</h4> </div>

                            <ol class="breadcrumb bg-transparent align-self-center m-0 p-0">
                                <li class="breadcrumb-item"><a href="<?php echo base_url('admin-dashboard')?>">Home</a></li>
                                <li class="breadcrumb-item active">Stocks</li>
                            </ol>
                        </div>
                    </div>
                </div>
                 <div class="row">
                    <div class="col-12 col-lg-6  col-xl-3 mt-3">
                        <a href="<?php echo base_url('stock-add')?>">
                        <div class="card text-white bg-danger" style="height: 100px">
                            
                            <div class="card-body" style="padding-top: 30px">
                                <h3 align="center">Add Stock</h3>
                            </div>
                        </div>
                    </a>
                    </div>
                    <div class="col-12 col-lg-6  col-xl-3 mt-3">
                        <a href="<?php echo base_url('view-stock')?>">
                        <div class="card text-white bg-primary" style="height: 100px">
                           <div class="card-body" style="padding-top: 30px">
                                <h3 align="center">View Stock</h3>
                            </div>
                        </div>
                    </a>
                    </div>
                        <div class="col-12 col-lg-6  col-xl-3 mt-3">
                        <a href="<?php echo base_url('overall-stock')?>">
                        <div class="card text-white bg-success" style="height: 100px">
                           <div class="card-body" style="padding-top: 30px">
                                <h3 align="center">Overall Stock</h3>
                            </div>
                        </div>
                    </a>
                    </div>
                     <div class="col-12 col-lg-6  col-xl-3 mt-3">
                        <a href="<?php echo base_url('reservation-stock')?>">
                        <div class="card text-white bg-secondary" style="height: 100px">
                           <div class="card-body" style="padding-top: 15px">
                                <h3 align="center">Stock Reservation</h3>
                            </div>
                        </div>
                    </a>
                    </div>
                 </div>